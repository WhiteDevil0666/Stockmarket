"""
app.py — AI Stock Predictor Dashboard
"""

# ── CRITICAL: Fix paths BEFORE any other import ──────────────
import os, sys
...
# ── Import pipeline modules at top level ─────────────────────
_PIPELINE_OK  = False
_PIPELINE_ERR = ""
try:
...
# ─────────────────────────────────────────────────────────────
# BUILT-IN UPDATE RUNNER
# ─────────────────────────────────────────────────────────────

def run_update_in_app(quick_mode: bool = False):
    if not _PIPELINE_OK:
        st.error(f"""
❌ **Import Error — pipeline modules could not be loaded.**

**Error:** `{_PIPELINE_ERR}`

**Most likely cause:** One or more files are missing from your GitHub repo.

**Please check that ALL these files exist in your repo:**
- `config.py`
- `data/__init__.py`
...
- `model/predictor.py`
    ...
    if quick_mode:
        ...
        st.info("⚡ Quick mode: 20 NSE + 5 crypto stocks.")
    else:
        nse_list    = NSE_STOCKS
    ...
    try:
        ...
        if not all_data:
            st.error("❌ No stock data downloaded. Check internet connection.")
            return False
        ...
        status.write("**Step 3/4** — Running sentiment analysis...")
        progress.progress(60, text="Analysing sentiment...")
        ...
        status.write("**Step 4/4** — Training ML models and predicting...")
        progress.progress(75, text="Training models...")
...
def main():
    ...
    # Show import error prominently if pipeline broken
    if not _PIPELINE_OK:
        st.error(f"""
### ❌ Setup Error — missing files

`{_PIPELINE_ERR}`

**Fix:** Make sure ALL files are uploaded to GitHub including:
`config.py` · `data/fetch_stocks.py` · `data/fetch_news.py` ·
`data/__init__.py` · `sentiment/analyzer.py` · `sentiment/__init__.py` ·
`model/predictor.py` · `model/__init__.py`
        """)
        return
    ...
    if df.empty:
        ...
        st.markdown("This is normal on first launch. Click a button below to generate your first predictions.")
        ...
        with col1:
            ...
            if st.button("▶ Run Quick Update", type="primary", use_container_width=True):
                with st.spinner("Running... please do not close this tab."):
        ...
        with col2:
            ...
            if st.button("▶ Run Full Update", use_container_width=True):
                with st.spinner("Running full update... please do not close this tab."):
        ...
        st.divider()
        st.info("After the first run, this app auto-updates every weekday at **6:30 AM IST** via GitHub Actions. You will never need to click this button again.")
        return
    ...
    with tab1:
        ...
        with col_pie:
            pie = px.pie(
                values=[(nse_df["signal"].str.startswith("BUY")).sum(),
            ...
            pie.update_layout(height=200, margin=dict(t=10,b=10,l=0,r=0),
        ...
        st.dataframe(filt[display].reset_index(drop=True),
    ...
    with tab2:
        ...
        if cdf.empty:
            ...
            st.dataframe(filt_c[display_c].reset_index(drop=True),
...
def render_stock_detail(df, symbol, currency="₹"):
    ...
    with c3:
        ...
        if hl:
            st.markdown(f"**Top Headline:** _{str(hl)[:120]}..._")
